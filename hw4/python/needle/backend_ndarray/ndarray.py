...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = tuple([sl.step * stride for sl, stride in zip(idxs, self._strides)])
        offset = sum([sl.start * stride for sl, stride in zip(idxs, self._strides)])

        return NDArray.make(new_shape, tuple(new_strides), self._device, self._handle, offset)
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        new_offset = sum([(self._shape[axis] - 1) * self._strides[axis] for axis in axes])
        return NDArray.make(self._shape, tuple(new_strides), self._device, self._handle, new_offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        new_shape = tuple([n + l + r for (l, r), n in zip(axes, self._shape)])
        out = self._device.full(new_shape, 0)
        indexes = tuple([slice(l, l + n) for (l, _), n in zip(axes, self._shape)])
        out[indexes] = self
        return out
        ### END YOUR SOLUTION
