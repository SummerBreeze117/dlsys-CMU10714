...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
   ...
   */
  /// BEGIN YOUR SOLUTION
  scalar_t *data_ptr = a.ptr + offset;
  // fixme: using 
  for (uint32_t i = 0; i < out->size; i ++) {
    uint32_t base = 1, pos = 0;
    for (int j = shape.size() - 1; j >= 0; j --) {
      pos += ((i / base) % shape[j]) * strides[j];
      base *= shape[j];
    }
    out->ptr[i] = data_ptr[pos];
  }
  /// END YOUR SOLUTION
}
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
   ...
   */
  /// BEGIN YOUR SOLUTION
  scalar_t *out_data_ptr = out->ptr + offset;
...
void ReduceSum(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (int i = 0; i < out->size; i ++) {
    ...
    size_t right = (i + 1) * reduce_size;
    // size_t right = std::min((i + 1) * reduce_size, a.size);
    out->ptr[i] = std::accumulate(a.ptr + left, a.ptr + right, (scalar_t)0);
  }
  /// END YOUR SOLUTION
}
