...
def xavier_normal(fan_in, fan_out, gain=1.0, **kwargs):
    """
        Fills the input Tensor with values, using a normal distribution.
        https://proceedings.mlr.press/v9/glorot10a/glorot10a.pdf
    """
    ### BEGIN YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, nonlinearity="relu", **kwargs):
    ...
    :param fan_out: dimensionality of output
    :param nonlinearity: the non-linear function
    :return: 2D tensors(fan_in * fan_out)
    """
    assert nonlinearity == "relu", "Only relu supported currently"
    ### BEGIN YOUR SOLUTION
    # recommended gain value for ReLU, gain = sqrt(2)
    gain = math.sqrt(2)
