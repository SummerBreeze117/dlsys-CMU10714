...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch_size=100):
    ...
    ### BEGIN YOUR CODE
    num_examples = y.size
    batch = (num_examples + batch_size - 1) // batch_size
    for i in range(batch):
        start = i * batch_size
        end = (i + 1) * batch_size
        x = X[start : end, :]
        yy = y[start : end]
        Z = np.exp(x @ theta) # (num_examples, num_classes)
        Z = Z / np.sum(Z, axis=1, keepdims=True)
        Y = np.zeros((batch_size, y.max() + 1))
        Y[np.arange(batch_size), yy] = 1
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch_size=100):
    ...
    for i in range(batch):
        ...
        Z1 = np.maximum(0, x @ W1) # (num_examples, hidden_dim)

        G2 = np.exp(Z1 @ W2) # (num_examples, num_classes)
        ...
        G1 = np.zeros_like(Z1) # (num_examples, hidden_dim)
        G1[Z1 > 0] = 1
        G1 = G1 * (G2 @ W2.T) 

        grad1 = x.T @ G1 / batch_size
...
def train_nn(X_tr, y_tr, X_te, y_te, hidden_dim = 500,
    ...
    print("| Epoch | Train Loss | Train Err | Test Loss | Test Err |")
    for epoch in range(epochs):
        nn_epoch(X_tr, y_tr, W1, W2, lr=lr, batch_size=batch)
