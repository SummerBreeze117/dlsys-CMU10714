...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        # sum up partial ajoints for its next nodes
        adjoint = sum_node_list(node_to_output_grads_list[node])
        node.grad = adjoint
        # leaf node
        if node.op is None:
            continue
        # compute partial ajoint for input nodes, len(node.inputs) <= 2
        partial_ajoints = node.op.gradient_as_tuple(adjoint, node)
        # mem partial ajoint in dict_list for input nodes
